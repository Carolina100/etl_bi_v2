...
#
# ESTE ARQUIVO IMPLEMENTA A CARGA DA ENTIDADE SX_FRENTE_D NO NOVO PADRAO
# INCREMENTAL.
#
...
#   ID_CLIENTE + CD_CORPORATIVO + CD_REGIONAL + CD_UNIDADE + CD_FRENTE
# ============================================================================

PIPELINE_NAME = "load_sx_frente_d"
OUTPUT_FOLDER_NAME = "sx_frente_d"
TARGET_TABLE = "SOLIX_BI.DS.SX_FRENTE_D"
SOURCE_NAME = "ORACLE.CDT_FRENTE"
TARGET_COLUMNS = [
    "ID_CLIENTE",
    "CD_CORPORATIVO",
    "CD_REGIONAL",
...
NATURAL_KEY_COLUMNS = ["ID_CLIENTE","CD_CORPORATIVO","CD_REGIONAL","CD_UNIDADE","CD_FRENTE"]
...
WITH base AS (
    SELECT
        ...
        r.CD_REGIONAL,
        u.CD_UNIDADE,
        ge.CD_GRUPO_EQUIPAMENTO AS CD_FRENTE,
        c.DESC_CORPORATIVO,
        r.DESC_REGIONAL,
        u.DESC_UNIDADE,
        ge.DESC_GRUPO_EQUIPAMENTO AS DESC_FRENTE,
        NVL(ge.FG_FRENTE_TRABALHO, 0) AS FG_FRENTE_TRABALHO,
        GREATEST(
            NVL(CAST(u.UPDATED_ON AS TIMESTAMP),  TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')),
            NVL(CAST(ge.UPDATED_ON AS TIMESTAMP), TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')),
            NVL(CAST(r.UPDATED_ON AS TIMESTAMP),  TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')),
            NVL(CAST(c.UPDATED_ON AS TIMESTAMP),  TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS'))
        ) AS SOURCE_UPDATED_ON
    FROM {name_owner}.CDT_UNIDADE u
    CROSS JOIN {name_owner}.CDT_GRUPO_EQUIPAMENTO ge
    INNER JOIN {name_owner}.CDT_REGIONAL r
        ON r.CD_REGIONAL = u.CD_REGIONAL
    INNER JOIN {name_owner}.CDT_CORPORATIVO c
        ON c.CD_CORPORATIVO = r.CD_CORPORATIVO
),
ranked AS (
    SELECT
        base.*,
