...
WITH base AS (
    SELECT
        ...
        ,NVL(go.desc_grupo_operac, '* não definido *') AS DESC_GRUPO_OPERACAO
        ,NVL(o.cd_grupo_parada, -1) AS CD_GRUPO_PARADA
        ,NVL(gp.desc_grupo_parada, '* não definido *') AS DESC_GRUPO_PARADA
        ,NVL(o.fg_tipo_operac, '-1') AS FG_TIPO_OPERACAO
        ,COALESCE(o.cd_processo, -1) AS CD_PROCESSO_TALHAO
        ,COALESCE(pt.desc_processo, '* não definido *') AS DESC_PROCESSO_TALHAO
        ,GREATEST(
