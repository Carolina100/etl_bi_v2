...
#
# ESTE ARQUIVO IMPLEMENTA A CARGA DA ENTIDADE SX_ORDEM_SERVICO_D NO NOVO PADRAO
# INCREMENTAL.
#
...
#   ID_CLIENTE + CD_ORDEM_SERVICO
# ============================================================================

PIPELINE_NAME = "load_sx_ordem_servico_d"
OUTPUT_FOLDER_NAME = "sx_ordem_servico_d"
TARGET_TABLE = "SOLIX_BI.DS.SX_ORDEM_SERVICO_D"
SOURCE_NAME = "ORACLE.CDT_ORDEM_SERVICO"
TARGET_COLUMNS = [
    "ID_CLIENTE",
    "CD_ORDEM_SERVICO",
    "CD_ORD_STATUS",
...
NATURAL_KEY_COLUMNS = ["ID_CLIENTE", "CD_ORDEM_SERVICO"]
...
WITH base AS (
    SELECT
        ...
        NVL(OS.CD_ORDEMSERVICO, -1) AS CD_ORDEM_SERVICO,
        NVL(OS.CD_ORD_STATUS, -1) AS CD_ORD_STATUS,
        NVL(CAST(SYS_EXTRACT_UTC(OS.CRIADO_EM) AS TIMESTAMP), TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) AS DT_CRIADO_EM,
        NVL(CAST(OS.DT_ABERTURA AS TIMESTAMP), TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) AS DT_ABERTURA,
        NVL(CAST(OS.DT_ENCERRAMENTO AS TIMESTAMP), TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) AS DT_ENCERRAMENTO,
        NVL(CAST(OS.DT_INICIO_EXEC AS TIMESTAMP), TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) AS DT_INICIO_EXEC,
        NVL(CAST(OS.DT_TERMINO_EXEC AS TIMESTAMP), TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) AS DT_TERMINO_EXEC,
        NVL(CAST(OS.DT_INICIO_PLAN_EXEC AS TIMESTAMP), TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) AS DT_INICIO_PLAN_EXEC,
        NVL(CAST(OS.DT_TERMINO_PLAN_EXEC AS TIMESTAMP), TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) AS DT_TERMINO_PLAN_EXEC,
        NVL(OS.DESC_OS, '* UNDEFINED *') AS DESC_OS,
        NVL(COS.DESCRICAO, '* UNDEFINED *') AS DESC_ORD_STATUS,
        NVL(OS.VL_LATITUDE, -1) AS VL_LATITUDE,
        NVL(OS.VL_LONGITUDE, -1) AS VL_LONGITUDE,
        NVL(OS.FG_ORIGEM, '-1') AS FG_ORIGEM,
        NVL(OS.FG_STATUS, -1) AS FG_STATUS,
        NVL(OS.VL_ORDEMSERVICO, '* UNDEFINED *') AS VL_ORDEM_SERVICO,
        -1 AS TICKET_NUMBER,
        NVL(CAST(COS.UPDATED_ON AS TIMESTAMP),
            TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
        ) AS SOURCE_UPDATED_ON
    FROM {name_owner}.CDT_ORDEMSERVICO OS
    LEFT JOIN {name_owner}.CDT_ORDEMSERVICO_STATUS COS
        ON OS.CD_ORD_STATUS = COS.CD_ID
),
ranked AS (
    SELECT
        base.*,
