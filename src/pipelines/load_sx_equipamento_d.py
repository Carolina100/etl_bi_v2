...
#
# ESTE ARQUIVO IMPLEMENTA A CARGA DA ENTIDADE SX_EQUIPAMENTO_D NO NOVO PADRAO
# INCREMENTAL.
#
...
#   ID_CLIENTE + CD_EQUIPAMENTO
# ============================================================================

PIPELINE_NAME = "load_sx_equipamento_d"
OUTPUT_FOLDER_NAME = "sx_equipamento_d"
TARGET_TABLE = "SOLIX_BI.DS.SX_EQUIPAMENTO_D"
SOURCE_NAME = "ORACLE.CDT_EQUIPAMENTO"
TARGET_COLUMNS = [
    "ID_CLIENTE",
    "CD_EQUIPAMENTO",
    "DESC_EQUIPAMENTO",
...
NATURAL_KEY_COLUMNS = ["ID_CLIENTE", "CD_EQUIPAMENTO"]
...
WITH status_atual AS (
    SELECT
        ...
        a.FG_STATUS,
        a.DT_HR_LOCAL
    FROM {name_owner}.LOG_HISTORICO_EQPTO_STATUS a
    INNER JOIN (
        SELECT
            MAX(DT_HR_LOCAL) AS DT_HR_LOCAL,
            CD_EQUIPAMENTO
        FROM {name_owner}.LOG_HISTORICO_EQPTO_STATUS
        GROUP BY CD_EQUIPAMENTO
    ) b
        ON a.DT_HR_LOCAL = b.DT_HR_LOCAL
...
base AS (
    SELECT
        ...
        e.CD_EQUIPAMENTO,
        e.DESC_EQUIPAMENTO,
        NVL(me.CD_MODELO_EQUIPAMENTO, -1) AS CD_MODELO_EQUIPAMENTO,
        NVL(me.DESC_MODELO_EQUIPAMENTO, ' not defined ') AS DESC_MODELO_EQUIPAMENTO,
        NVL(te.CD_TP_EQUIPAMENTO, -1) AS CD_TIPO_EQUIPAMENTO,
        NVL(te.DESC_TP_EQUIPAMENTO, ' not defined ') AS DESC_TIPO_EQUIPAMENTO,
        DECODE(lg.FG_STATUS, 'I', 'INACTIVE', 'ACTIVE') AS DESC_STATUS,
        NVL(e.TP_USO_EQUIPAMENTO, -1) AS TP_USO_EQUIPAMENTO,
        GREATEST(
            NVL(CAST(e.UPDATED_ON AS TIMESTAMP), TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')),
            NVL(CAST(me.UPDATED_ON AS TIMESTAMP), TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')),
            NVL(CAST(te.UPDATED_ON AS TIMESTAMP), TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')),
            NVL(CAST(lg.DT_HR_LOCAL AS TIMESTAMP), TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS'))
        ) AS SOURCE_UPDATED_ON
    FROM {name_owner}.CDT_EQUIPAMENTO e
    LEFT JOIN {name_owner}.CDT_MODELO_EQUIPAMENTO me
        ON me.CD_MODELO_EQUIPAMENTO = e.CD_MODELO_EQUIPAMENTO
    LEFT JOIN {name_owner}.CDT_TIPO_EQUIPAMENTO te
        ON te.CD_TP_EQUIPAMENTO = e.CD_TP_EQUIPAMENTO
    LEFT JOIN status_atual lg
        ON lg.CD_EQUIPAMENTO = e.CD_EQUIPAMENTO
    WHERE te.CD_TP_EQUIPAMENTO IN ('101', '103', '104', '105', '106', '107', '108', '120', '121', '122')
),
ranked AS (
    SELECT
        base.*,
