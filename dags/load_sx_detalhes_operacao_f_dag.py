...
DAG_ID = "load_sx_detalhes_operacao_f_dag"
PIPELINE_DESCRIPTION = "Carga PostgreSQL -> Snowflake DS -> dbt DW para SX_DETALHES_OPERACAO_F"
TAGS = ["postgres", "snowflake", "dbt", "ds", "dw", "sx_detalhes_operacao_f", "fact"]
DBT_PROJECT_DIR = PROJECT_ROOT / "dbt" / "solix_dbt"
DBT_SELECT_MODELS = ["stg_ds__sx_detalhes_operacao_f", "fct_sx_detalhes_operacao_f"]
PIPELINE_TASK_ID = "run_ds_pipeline"
DW_PIPELINE_NAME = "dw_sx_detalhes_operacao_f_dbt"
DW_SOURCE_NAME = "DBT.SOLIX_BI.DS.SX_DETALHES_OPERACAO_F"
DW_TARGET_NAME = "SOLIX_BI.DW.SX_DETALHES_OPERACAO_F"
...
def load_sx_detalhes_operacao_f_dag():
    ...
    def resolve_clientes(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        resolved_payload["id_clientes"] = [int(row[0]) for row in rows]
        if not resolved_payload["id_clientes"]:
    ...
    def run_ds_pipeline_task(request: dict[str, Any]) -> dict[str, Any]:
        from src.pipelines.load_sx_detalhes_operacao_f import run_pipeline
        ...
        try:
            ...
            logger.exception("Falha na camada DS para id_cliente=%s.", request["id_cliente"])
            return {
                "id_cliente": int(request["id_cliente"]),
                "status": "ERROR",
                "result": None,
    ...
    def summarize_ds_results(ds_results: list[dict[str, Any]]) -> dict[str, Any]:
        successful_items = [item for item in list(ds_results) if item.get("status") == "SUCCESS"]
        failed_items = [item for item in list(ds_results) if item.get("status") == "ERROR"]
        return {
            ...
            "successful_results": [
                item["result"] for item in successful_items if isinstance(item.get("result"), dict)
            ],
    ...
    def run_dw_dbt_task(ds_summary: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
        ...
        successful_results = list(ds_summary.get("successful_results", []))
        ...
        audit_dt_inicio = min(str(result["window_start"]) for result in successful_results)
        audit_dt_fim = max(str(result["window_end"]) for result in successful_results)
        upstream_batch_ids = [
            str(result.get("batch_id")) for result in successful_results if result.get("batch_id")
        ]
        upstream_rows_loaded = sum(int(result.get("rows_loaded") or 0) for result in successful_results)
        ...
        try:
            ...
            audit_repository.insert_audit_event(
                ...
                details=(
                    ...
                    f"successful_clientes={successful_clientes}, failed_clientes={failed_clientes}, "
                    f"load_mode={payload['load_mode']}, upstream_batch_ids={upstream_batch_ids}, "
                    f"rows_inserted_total={rows_inserted_total}, rows_updated_total={rows_updated_total}, "
                    f"rows_deleted_total={rows_deleted_total}, rows_affected_total={rows_affected_total}."
                ),
                id_cliente=audit_id_cliente,
                dt_inicio=audit_dt_inicio,
                dt_fim=audit_dt_fim,
            ...
            for model_result in run_results.get("model_results", []):
            ...
            try:
                ...
                audit_repository.update_batch_error(batch_id=dw_batch_id, error_message=str(exc))
            except Exception:
...
load_sx_detalhes_operacao_f = load_sx_detalhes_operacao_f_dag()
