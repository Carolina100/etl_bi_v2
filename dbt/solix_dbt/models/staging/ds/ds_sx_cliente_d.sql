...
{% set RAW_SOURCE_TABLE = 'SOLIX_BI.RAW.VW_SX_CLIENTE_D' %}
...
current_target as (
    ...
    select
        ...
        cast(null as varchar) as NOME_CLIENTE,
        cast(null as varchar) as OWNER_CLIENTE,
        cast(null as varchar) as SERVER_CLIENTE,
...
changed_or_new as (
    select
        ...
        case
            ...
            when coalesce(t.NOME_CLIENTE, '') <> coalesce(s.NOME_CLIENTE, '')
                then convert_timezone('UTC', current_timestamp())::timestamp_ntz
            when coalesce(t.OWNER_CLIENTE, '') <> coalesce(s.OWNER_CLIENTE, '')
                then convert_timezone('UTC', current_timestamp())::timestamp_ntz
            when coalesce(t.SERVER_CLIENTE, '') <> coalesce(s.SERVER_CLIENTE, '')
                then convert_timezone('UTC', current_timestamp())::timestamp_ntz
    ...
    where
        ...
        or coalesce(t.NOME_CLIENTE, '') <> coalesce(s.NOME_CLIENTE, '')
        or coalesce(t.OWNER_CLIENTE, '') <> coalesce(s.OWNER_CLIENTE, '')
        or coalesce(t.SERVER_CLIENTE, '') <> coalesce(s.SERVER_CLIENTE, '')
